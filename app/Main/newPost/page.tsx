...
export default function Page() {
  ...
  async function submitPost() {
    fetch(`http://localhost:3002/posts/${subjectSelector}`, {
      ...
      body: JSON.stringify({
        userWalletAddress: "0x8eD60F263CaA5c45739CdBA594ebF137B81f386A", // 이후에 추가
        userName: "택용 테스트",
        postTitle: title,
        postContent: content,
      }),
  ...
  return (
    <div className=" mt-16  w-10/12 m-auto mb-8  ">
      <p className="text-3xl  mb-12">글 올리기</p>

      <div className="flex justify-center gap-8">
        <div className="w-4/6">
          <div className="border-2 border-black py-8 rounded-md">
            <form id="board" className="flex flex-col gap-6">
              <div className="flex w-inherit">
                <div className="w-10/12 flex gap-8">
                  <label htmlFor="title" className="text-xl ml-8">
                    제목
                  </label>
                  <input
                    className="w-10/12 border border-black left-2"
                    id="title"
                    name="title"
                    type="text"
                    onChange={(e) => {
              ...
              <div className="w-inherit flex gap-8">
                <label htmlFor="text" className="text-xl ml-8">
                  내용
                </label>
                <textarea
                  id="content"
                  name="content"
                  className="left-8 w-10/12 h-80 border border-black"
                  onChange={(e) => {
              ...
              <button
                type="button"
                className="border border-black self-center px-2 py-1 rounded-xl"
        ...
        <div className="w-2/6 border border-black rounded-md">
          <ul>
